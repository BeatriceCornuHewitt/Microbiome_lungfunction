...
# Check the composition of the different samples types - not per niche
create_ordered_bar_n <- function(ps, n) {
  
  # Convert phyloseq object to relative abundance (RA)
  ps_RA <- to_RA(ps)  # Assuming to_RA() is a function that converts to relative abundance
  
  # Get the OTU table from the phyloseq object and convert it to a matrix
  otu_RA_m <- as(otu_table(ps_RA), "matrix")
  ...
  # Prepare the top n most abundant ASVs for the bar plot
  ps_RA %>%
    prep_bar(n = n) %>%
    mutate(sample_id = fct_relevel(sample_id, hc$labels[hc$order])) %>%
    create_bar(df_topn = ., n = n, ncol_legend = 1, name_legend = "ASV") +
...
create_tax_names <- function(tax_table) {
  ...
  tax_names <- apply(tax_table, 1, function(x) {
    genus <- ifelse("genus" %in% names(x), x[["genus"]], NA)
    species <- ifelse("species" %in% names(x), x[["species"]], NA)
    
    if(!is.na(genus) & !is.na(species)) {
      name <- paste0(genus, "_", species)
    } else if (!is.na(genus)) {
      name <- genus
...
create_tax_names <- function(tax_table) {
  tax_names <- apply(tax_table, 1, function(x) {
    ...
    # Extract genus and species columns safely (lowercase colnames assumed)
    genus <- if("genus" %in% names(x_no_na)) x_no_na["genus"] else NA_character_
    species <- if("species" %in% names(x_no_na)) x_no_na["species"] else NA_character_
    
    # Helper: get highest taxonomic rank available excluding species
    get_high_rank <- function(x) {
      ranks <- c("genus", "family", "order", "class", "phylum", "kingdom")
      for (r in ranks) {
        if (r %in% names(x) && !is.na(x[r]) && x[r] != "") return(x[r])
      }
      return(NA_character_)
    }
    
    # Check if species is multiple species (has slash '/')
    multiple_species <- !is.na(species) && grepl("/", species)
    
    if (!is.na(genus) && genus != "") {
      if (!is.na(species) && species != "" && !multiple_species) {
        # Single species, name: genus_species
        name <- paste(genus, species, sep = "_")
      } else {
        # Multiple species or no species, just genus
        name <- genus
      }
    } else {
      # No genus, fallback to next highest rank available (excluding species)
      high_rank <- get_high_rank(x_no_na)
      name <- if (!is.na(high_rank)) high_rank else "Unclassified"
    }
  ...
  tax_names <- gsub("\\[|\\]|\\(|\\)", "", tax_names)
  
  # Append row numbers
  tax_names <- paste0(tax_names, "_", seq_len(nrow(tax_table)))
